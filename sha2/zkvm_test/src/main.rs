...
fn main() -> Result<()> {
    ...
    // Identify sha2 chip AIRs by matching name prefixes
    let sha2_airs: Vec<(usize, &str)> = air_names
        .iter()
        .enumerate()
        .filter(|(_, name)| SHA2_AIR_PREFIXES.iter().any(|p| name.contains(p)))
        .map(|(idx, name)| (idx, name.as_str()))
        .collect();

    if sha2_airs.is_empty() {
        return Err(eyre!(
            "No sha2-related AIRs found. Is the sha2 extension enabled in openvm.toml?"
        ));
    }
    ...
    // Verify that at least one sha2 AIR has a non-zero trace height,
    // which confirms the custom sha2 opcodes were actually executed.
    let mut any_sha2_used = false;

    for (seg_idx, segment) in segments.iter().enumerate() {
        println!("Segment {seg_idx} (num_insns: {}):", segment.num_insns);
        for &(air_idx, air_name) in &sha2_airs {
    ...
    if any_sha2_used {
        println!("PASS: SHA-2 chips have non-zero trace heights.");
        Ok(())
    } else {
        Err(eyre!(
            "FAIL: All sha2 chip trace heights are zero.\n\
             The sha2 chips are not being used; the patch may not be working correctly."
        ))
    }
}
